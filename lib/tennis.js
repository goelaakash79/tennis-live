...
import { resolveTournamentVenue } from '@/lib/tournament-venues';
 ...
 * - Olympic tennis
 * - Major Indian opens (Bengaluru, Karnataka, Chennai, Delhi, etc.) — singles + doubles
 *
 ...
 * All other events are singles-only.
 */
...
/** Server-side in-memory cache: player_key (number) → { flagEmoji, flagCode, rank } */
...
/** Named Indian tour events and city opens (excludes Indian Wells). */
const INDIAN_MAJOR_OPEN_PHRASES = [
  'bengaluru open',
  'bangalore open',
  'karnataka open',
  'chennai open',
...
const INDIA_FLAG = { flagEmoji: '🇮🇳', flagCode: 'IN' };
...
function isIndianWellsFalsePositive(n) {
  return n.includes('indian wells') || n.includes('bnp paribas');
}

/** ATP/WTA/ITF/Challenger events at major Indian venues (singles + doubles). */
function isIndianMajorTournament(n) {
  if (isIndianWellsFalsePositive(n)) return false;
  if (textMatchesAny(n, INDIAN_MAJOR_OPEN_PHRASES)) return true;
  if (/\bopen\b/.test(n) && textMatchesAny(n, INDIAN_CITY_PHRASES)) return true;
  if (/\bindia\b/.test(n) && /\b(open|challenger|atp|wta|itf|futures|125|series)\b/.test(n)) {
    return true;
  }
  if (
    textMatchesAny(n, INDIAN_CITY_PHRASES) &&
    /\b(open|challenger|championship|masters)\b/.test(n)
  ) {
    return true;
  }
  return false;
...
function isIncludedTournament(n) {
  return isDefaultTournamentSet(n) || isIndianMajorTournament(n);
}

function isDoublesRow(row) {
  return String(row.event_type_type || '').toLowerCase().includes('doubles');
}
...
function buildAllowedTournamentKeySet(catalogRows) {
  ...
  for (const t of catalogRows) {
    ...
    if (!n.trim()) continue;
    if (!isIncludedTournament(n)) continue;
...
function includeMatchRow(row, allowedTournamentKeys) {
  ...
  return isIncludedTournament(n);
}

/**
 * Grand slams / Masters / Olympics: singles only.
 * Indian majors: singles and doubles.
 */
function shouldIncludeMatchRow(row, allowedTournamentKeys) {
  if (!includeMatchRow(row, allowedTournamentKeys)) return false;
  if (isDoublesRow(row)) {
    return isIndianMajorTournament(rowFilterHaystack(row));
  }
  return true;
}
...
function inferTourFromEventType(eventType) {
  ...
  if (n.includes('women') || n.includes('wta') || n.includes('girls')) return 'WTA';
  if (n.includes('men') || n.includes('atp') || n.includes('boys') || n.includes('challenger')) {
    return 'ATP';
  }
  return 'ATP';
}
...
/** Last set row in the point-by-point feed (current set for live scoring). */
function getLastPointByPointSetRow(pointbypoint) {
...
function currentGameFromPointByPoint(pointbypoint) {
  const lastSetRow = getLastPointByPointSetRow(pointbypoint);
...
/** Names that usually indicate a match court / stadium show court, not a city. */
const STADIUM_COURT_RE =
  /(court|cent(re|er)|stadium|arena|field|show|grandstand|garden|tablo|(philippe|suzanne|lenglen|chatrier|rod\s*laver|arthur\s*ashe)|pista)/i;

function looksLikeStadiumCourtName(s) {
  if (!String(s || '').trim()) return false;
  return STADIUM_COURT_RE.test(s);
}

/** `event_location` is often "City, Country" — treat as geography, not court. */
function looksLikeCityOrRegion(s) {
  return /,\s*\S/.test(String(s || '').trim());
}

/**
 * Stadium court / on-site court label (green chip), not city.
 * API-Tennis commonly puts the court in `event_place`; `event_location` is often the city — only use it when it looks like a court name.
 */
function formatCourtFromRow(m) {
  const place = String(m.event_place || '').trim();
  const loc = String(m.event_location || '').trim();
  const alt = String(m.event_stadium || m.event_court || m.court_name || '').trim();

  if (looksLikeStadiumCourtName(place)) return place;
  if (looksLikeStadiumCourtName(loc)) return loc;
  if (looksLikeStadiumCourtName(alt)) return alt;
  if (place && (!loc || looksLikeCityOrRegion(loc))) {
    if (looksLikeBareShortPlaceName(place) && !looksLikeStadiumCourtName(place)) return '';
    return place;
  }
  return '';
}

/** Avoid treating a lone short token (e.g. city) as a court when it has no court cues. */
function looksLikeBareShortPlaceName(s) {
...
function mapApiTennisMatch(m) {
  ...
  const isDoubles = isDoublesRow(m);
  const tournHaystack = normalizeFilterText(
    [tournamentName, m.event_type_type, m.event_place, m.event_location].filter(Boolean).join(' '),
  );
  const isIndian = isIndianMajorTournament(tournHaystack);
  const sets =
  ...
  const venue =
    formatCourtFromRow(m) ||
    resolveTournamentVenue({
      tournamentName,
      tournamentKey: m.tournament_key,
  ...
  return {
    ...
    tournament: {
      ...
      flag: isIndian ? INDIA_FLAG.flagEmoji : flagForTournament(tournamentName),
      tour,
    },
    status,
    isDoubles,
...
function extractSinglesRank(stats) {
  if (!Array.isArray(stats) || !stats.length) return null;
  const curYear = new Date().getUTCFullYear();
  const singles = stats.filter((s) => {
    const t = String(s.type || '').toLowerCase();
    return t === 'singles' || t.includes('single');
  });
  const rows = singles.length ? singles : stats;
  const ranked = rows
    .map((s) => ({
      year: parseInt(String(s.season || ''), 10),
      rank: parseInt(String(s.rank || '').trim(), 10),
    }))
    .filter((x) => Number.isFinite(x.rank) && x.rank > 0);
  if (!ranked.length) return null;
  const thisYear = ranked.find((x) => x.year === curYear);
  if (thisYear) return thisYear.rank;
  ranked.sort((a, b) => b.year - a.year);
...
async function resolvePlayerFlag(playerKey, apiKey, apiHost) {
  ...
  try {
    ...
    if (!player) return;
    const rank = extractSinglesRank(player.stats);
    const flag = player.player_country
      ? countryNameToFlag(player.player_country)
      : { flagEmoji: null, flagCode: '' };
    _playerFlagCache.set(k, { ...flag, rank: rank ?? null });
  } catch {
    clearTimeout(timer);
  }
}
...
async function fetchTennisFromApiTennisOnly() {
  ...
  for (const row of liveRows) {
    if (!shouldIncludeMatchRow(row, allowedTournamentKeys)) continue;
  ...
  // Player flags: singles via get_players; Indian doubles use 🇮🇳 (team keys don't map cleanly).
  ...
  for (const match of [...live, ...upcoming, ...past]) {
    if (match.isDoubles) continue;
    const keys = eventPlayerKeys.get(match.id);
    if (!keys) continue;
    if (keys.p1) uniquePlayerKeys.add(keys.p1);
    if (keys.p2) uniquePlayerKeys.add(keys.p2);
  }
  await resolvePlayerFlagsAll(uniquePlayerKeys, apiKey, apiHost);

  for (const match of [...live, ...upcoming, ...past]) {
    if (match.isDoubles) {
      match.p1.flagEmoji = INDIA_FLAG.flagEmoji;
      match.p1.flagCode = INDIA_FLAG.flagCode;
      match.p2.flagEmoji = INDIA_FLAG.flagEmoji;
      match.p2.flagCode = INDIA_FLAG.flagCode;
      continue;
    }
    const keys = eventPlayerKeys.get(match.id);
    if (!keys) continue;
    ...
    if (f1) {
      match.p1.flagEmoji = f1.flagEmoji;
      match.p1.flagCode = f1.flagCode;
      if (f1.rank != null) match.p1.rank = f1.rank;
    }
    if (f2) {
      match.p2.flagEmoji = f2.flagEmoji;
      match.p2.flagCode = f2.flagCode;
      if (f2.rank != null) match.p2.rank = f2.rank;
    }
  }
...
const fetchTennisFromApiTennisOnlyCached = unstable_cache(
  () => fetchTennisFromApiTennisOnly(),
  ['tennis-api-aggregate'],
  { revalidate: 8, tags: ['tennis-matches'] },
);
...
export async function fetchTennis() {
  ...
  return fetchTennisFromApiTennisOnlyCached();
}
